...
tomodrgn convergence_vae output/05_vae_both_sim 49 --Apix 13.1 --random-seed 42
...
tomodrgn analyze output/05_vae_both_sim 49 --Apix 13.1 --ksample 2
...
tomodrgn analyze output/07_vae_classE_sim 49 --Apix 13.1 --ksample 2

### EVAL_VOL ###
# Test eval_vol for all particles --> should get director with 144 unique volumes
tomodrgn eval_vol output/05_vae_both_sim/weights.pkl -c output/05_vae_both_sim/config.pkl -o output/05_vae_both_sim/eval_vol_allz --zfile output/05_vae_both_sim/z.pkl --Apix 13.1
...
tomodrgn subtomo2chimerax data/10076_both_32_sim_vols.star -o output/05_vae_both_sim/vol_allz_chimerax.cxc --tomoname both.tomostar --star-apix-override 10 --vols-dir output/05_vae_both_sim/eval_vol_allz --coloring-labels data/ptcl_labels_D0_E1.pkl
...
### EVAL_IMAGES ###
# Test eval_images with same images + weights as 07_vae_classE_sim --> z_all.pkl should be very close to z.pkl even with undertrained network 07 and different random image ordering
tomodrgn eval_images data/10076_classE_32_sim.star output/07_vae_classE_sim/weights.pkl -c output/07_vae_classE_sim/config.pkl --out-z output/07_vae_classE_sim/eval_images/z_all.pkl
# Test eval_images, should eventually compare this output (z_all.pkl) to that of 05_vae_both_sim/z.pkl for similarity given random sampled ntilts
tomodrgn eval_images data/10076_both_32_sim.star output/05_vae_both_sim/weights.pkl -c output/05_vae_both_sim/config.pkl --out-z output/05_vae_both_sim/eval_images/z_all.pkl

### GRAPH_TRAVERSAL ###
# Test graph_traversal --> should either report path indices and distance between neighbors or find no path
tomodrgn graph_traversal output/05_vae_both_sim/z.0.pkl --anchors 137 10 20 -o output/05_vae_both_sim/graph_traversal/path.txt --out-z output/05_vae_both_sim/graph_traversal/z.path.txt

### PC_TRAVERSAL ###
# Test pc_traversal
tomodrgn pc_traversal output/05_vae_both_sim/z.pkl -o output/05_vae_both_sim/pc_traversal
tomodrgn pc_traversal output/05_vae_both_sim/z.pkl -o output/05_vae_both_sim/pc_traversal --use-percentile-spacing
