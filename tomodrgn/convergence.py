...
def plot_loss(runlog: str,
              outdir: str,
              plot_format: Literal['png', 'svg']) -> None:
    """
    ...
    outpath = f'{outdir}/plots/00_total_loss.{plot_format}'
    plt.savefig(outpath, dpi=300)
    log(f'Saved total loss plot to {outdir}/plots/00_total_loss.{plot_format}')
    plt.close()
...
def plot_latent_pca(workdir: str,
    ...
    plt.savefig(f'{outdir}/plots/01_encoder_pcs.{plot_format}', dpi=300)
    log(f'Saved PCA plots to {outdir}/plots/01_encoder_pcs.{plot_format}', )
    plt.close()
...
def plot_latent_umap(workdir: str,
    ...
    plt.savefig(f'{outdir}/plots/02_encoder_umaps.{plot_format}', dpi=300)
    log(f'Saved UMAP distribution plot to {outdir}/plots/02_encoder_umaps.{plot_format}')
    plt.close()
...
def encoder_latent_shifts(workdir: str,
    ...
    fig.tight_layout()
    plt.savefig(f'{outdir}/plots/03_encoder_latent_vector_shifts.{plot_format}', dpi=300)
    log(f'Saved latent vector shifts plots to {outdir}/plots/03_encoder_latent_vector_shifts.{plot_format}')
    plt.close()
...
def sketch_via_umap_local_maxima(outdir: str,
    ...
    plt.tight_layout()
    plt.savefig(f'{outdir}/plots/04_decoder_UMAP-sketching.{plot_format}', dpi=300)
    log(f'Saved latent sketching plot to {outdir}/plots/04_decoder_UMAP-sketching.{plot_format}')
    plt.close()
...
def follow_candidate_particles(workdir: str,
    ...
    Saves plot.{plot_format} tracking representative latent encodings through epochs and latent.txt of representative latent encodings for each epoch
    ...
    plt.savefig(f'{outdir}/plots/05_decoder_maxima-sketch-consistency.{plot_format}', dpi=300)
    log(f'Saved plot tracking representative latent encodings through epochs {epochs} to {outdir}/plots/05_decoder_maxima-sketch-consistency.{plot_format}')
    plt.close()
...
def calc_ccs_pairwise_epochs(outdir: str,
    ...
    plt.tight_layout()
    plt.savefig(f'{outdir}/plots/06_decoder_CC.{plot_format}', dpi=300)
    log(f'Saved map-map correlation plot to {outdir}/plots/06_decoder_CC.{plot_format}')
    plt.close()
...
def calc_fscs_pairwise_epochs(outdir: str,
    ...
    for (i, ax) in enumerate(axes.ravel()):
        try:
            ...
            ax.axis('off')
    plt.tight_layout()
    plt.savefig(f'{outdir}/plots/07_decoder_FSC.{plot_format}', dpi=300)
    log(f'Saved map-map FSC plot to {outdir}/plots/07_decoder_FSC.{plot_format}')
    plt.close()
    ...
    ax.legend(labels, ncol=3, fontsize='x-small')
    plt.tight_layout()
    plt.savefig(f'{outdir}/plots/08_decoder_FSC-nyquist.{plot_format}', dpi=300)
    log(f'Saved map-map FSC (Nyquist) plot to {outdir}/plots/08_decoder_FSC-nyquist.{plot_format}')
    plt.close()
...
def calc_ccs_alltoall_intraepoch(outdir: str,
    ...
    for i in range(len(epochs)):
        ...
        plt.savefig(f'{outdir}/plots/09_pairwise_CC_matrix_epoch-{i}.{plot_format}', dpi=300, transparent=True, bbox_inches='tight')
        plt.close()
...
def calc_ccs_alltogroundtruth(outdir: str,
    ...
    for i in range(len(epochs)):
        ...
        sns.clustermap(df, annot=True, fmt='0.2f', figsize=(6, 6), vmin=0, vmax=1.0, row_cluster=False)
        plt.tight_layout()
        plt.savefig(f'{outdir}/plots/10_groundtruth_CC_matrix_epoch-{i}.{plot_format}', dpi=300)
        plt.close()
