...
def add_args(parser):
    ...
    parser.add_argument('-o', '--outfile', type=os.path.abspath, required=True, help='Output script to be opened in ChimeraX')
    ...
    group.add_argument('--vols-render-level', type=float, default=0.1, help='Isosurface level to render all tomoDRGN reconstructed volumes in ChimeraX')
...
def main(args):
    ...
    assert np.all(rots_cols in ptcl_star.blocks['data_'].columns), f'`{rots_cols} columns not found in starfile `data_` block'
    assert np.all(trans_cols in ptcl_star.blocks['data_'].columns), f'{trans_cols} columns not found in starfile `data_` block'
    ...
    if args.star_apix_override is not None:
        assert '_rlnPixelSize' in df_tomo.columns, 'Could not find `_rlnPixelSize` column in input starfile. Make sure it is present, or use --star-apix-override'
        star_apix = float(df_tomo['_rlnPixelSize'][0])
    else:
    ...
    all_vols = [vol for vol in all_vols if vol.endswith('.mrc')]
    vol, header = mrc.parse_mrc(all_vols[0])
    vol_box = vol.shape[0]
    vol_apix = header.get_apix() if args.vols_apix_override is not None else args.vols_apix_override
    ...
    if args.coloring_labels is not None:
        # load the labels file and filter by ind.pkl + tomoname
    ...
    with open(args.outfile, 'w') as f:
        for i in range(len(df_tomo)):
            ...
            rot = utils.R_from_relion(eulers_relion)
            ...
            if args.coloring_labels is not None:
                ...
                f.write(f'color #{i+1:d} rgba{tuple(labels_rgba[0])}\n')
