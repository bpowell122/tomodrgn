...
def add_args(_parser):
    ...
    _parser.add_argument('--tomo-id-col-override',
                         type=str,
                         help='Name of column in input starfile to filter by --tomoname')
    _parser.add_argument('--star-angpix-override',
                         type=float,
                         help='Override pixel size of input particle_volumeseries starfile (units Å/px)')

    group = _parser.add_argument_group('Volume specification options - generate unique-per-particle tomoDRGN volumes. Required if `--mode volumes`.')
    ...
    group.add_argument('--skip-vol',
                       ...
                       help='Skip generation of volumes. Should only be used if volmes have been previously generated by this script. '
                            'Useful for tweaking coloring_labels or colormap. Is not valid if any volume generation parameters are changed.')

    group = _parser.add_argument_group('Volume specification options - single pregenerated volume. Required if `--mode volume`.')
    group.add_argument('--vol-path',
                       type=os.path.abspath,
                       help='Path to single consensus volume to map back at each particle location in tomogram.')

    group = _parser.add_argument_group('Volume specificaiton options - ChimeraX spherical markers. Required if `--mode markers`.')
    group.add_argument('--marker-radius-angstrom',
                       type=float,
                       help='Radius of ChimeraX markers (units: Å).')

    group = _parser.add_argument_group('ChimeraX rendering options')
...
def validate_particles_for_rendering(mode: Literal['volumes', 'volume', 'markers'],
    ...
    if mode == 'volumes' or mode == 'volume':
        ...
        assert len(vol_paths) == num_ptcls_this_tomo
...
def write_mapback_script(df_one_tomo: pd.DataFrame,
    ...
    with open(f'{outdir_one_tomo}/mapback.cxc', 'w') as f:
        for i in range(len(df_one_tomo)):
            ...
            if mode == 'volumes':
                f.write(f'open "{os.path.basename(vol_paths[0])}"\n')
            elif mode == 'volume':
            ...
            coord_px = df_one_tomo.iloc[i][coords_cols].to_numpy(dtype=np.float32)
            ...
            if '_rlnOriginXAngst' in df_one_tomo.columns:
                ...
                coord_ang -= shift_ang
            elif '_rlnOriginX' in df_one_tomo.columns:
                shift = df_one_tomo.iloc[i]['_rlnOriginXAngst', '_rlnOriginYAngst', '_rlnOriginZAngst'].to_numpy(dtype=np.float32)
